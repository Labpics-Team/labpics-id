...
| decorative `--lab-ink-faint` | ink 52% над карточкой | `#9A9A9D` | exempt — **только декор/логотип**, никогда текст (§3.2) |
...
Точная 52%-доля мока при этом **не потеряна**: она сохранена как
`--lab-ink-faint` — токен строго декоративного назначения (тонировка
логотипа-водяного знака по исключению WCAG 1.4.3 для логотипов,
декоративная графика). Для текста любого размера он запрещён; F1 пинит
значение (`#9A9A9D` light), чтобы вкус мока не дрейфовал.
